...
function showMemory(){
  textSize(scaler);
  fill(255);
  text("Memory position", width - scaler * 9, height - scaler * 2.5);
  if(memory.length > 0){
...
function showUnitedLetters(){
  fill(255);
  if(autoMode){
    text("United letters : " + memory[memoryPosition].unitedLetters , width - scaler * 9, height - scaler * 4.5);
  }else{
    let myNewTemp = 0;
    ...
    text("United letters : " + myNewTemp , width - scaler * 9, height - scaler * 4.5);
  }
}